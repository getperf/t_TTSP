...
#	$data_info->skip_header( $in );
	while (my $line = <$in>) {
	    ...
	    if ($parse_block eq 'INFO' && $row > 1) {
	    	if ($line=~/^(.+?)\s+: (.+?)$/) {
				...
				# ---                   0       ON      0       1     8       256
	    		if ($line!~/time/ && ($cols == 4 || $cols == 7) ) {
		    		@csvs = ('', '', '', @csvs) if ($cols == 4);
		    		my ($device, $ldisk, $wbc, $raid_gr, $level, $hdd, $stripe) = @csvs;
		    		# Parse device ; c4t20000015B74FEA79d20(c5t20000015B74EEA79d20)
	    			$luns{$controller}{$ldisk} = {ldisk => $ldisk, row => $row};
	    			$raid_groups{$controller}{$raid_gr} = {level => $level, hdd => $hdd, row => $row};
	...
	for my $controller(sort keys %controllers) {
		...
		$data_info->regist_node($host, 'SC3000', 'info/model', \%infos);

		for my $metric(qw/wbc summary read_elapse write_elapse read_size write_size/) {
			$data_info->regist_metric($host, 'SC3000', $metric, $controller_headers{$metric});
			my $output_file = "SC3000/${host}/${metric}.txt";
		...
		for my $lun(sort {$luns{$controller}{$a}{row} <=> $luns{$controller}{$b}{row}}
			...
			if (exists($luns{$controller}{$lun}{ldisk})) {
				...
				for my $device_metric(qw/lun_summary lun_read_elapse lun_write_elapse lun_read_size lun_write_size/) {
					$data_info->regist_device($host, 'SC3000', $device_metric, $lun2, $lun_text, 
											  $lun_headers{$device_metric});
					my $output_file = "SC3000/${host}/device/${device_metric}__${lun2}.txt";
		...
		for my $raid_group(sort {$raid_groups{$controller}{$a}{row} <=> $raid_groups{$controller}{$b}{row}}
			...
			for my $device_metric(qw/raid_summary raid_read_size raid_write_size/) {
				$data_info->regist_device($host, 'SC3000', $device_metric, $raid_group2, $raid_group_text, 
										  $raid_group_headers{$device_metric});
				my $output_file = "SC3000/${host}/device/${device_metric}__${raid_group2}.txt";
